    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (StringUtils.isNotEmpty(defaultStr)) {
            ...
            if (defaultStr.length() <= maxlength) {
                numberTips.setText("You can also enter it" + (maxlength - defaultStr.length()) + "The word ("
    ...
    TextWatcher mTextWatcher = new TextWatcher() {
                                 ...
                                 public void afterTextChanged(Editable s) {
                                     ...
                                     if (s.toString().length() > maxlength) {
                                         toast("The number of words you have entered has exceeded the limit！", Toast.LENGTH_LONG);
                                         s.delete(editStart - 1, editEnd);
                                         int tempSelection = editStart;
                                         contentView.setText(s);
                                         contentView.setSelection(tempSelection);
                                     }
                                     numberTips.setText("You can also enter it"
                                                        + (maxlength - s.toString().length())
                                                        + "The word  (" + s.toString().length() + "/"
                                                        + maxlength + ")");
                                 }
                             };
}
